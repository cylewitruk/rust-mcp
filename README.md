...
- `rmcp` streamable HTTP transport mounted at `/mcp` with `ping`, `index.sync_crates`, `index.status`, `index.refresh`, `crate.search`, and `crate.intel` tools.
...
- `index.refresh`: refreshes index scope (`crate` and `all` implemented; `security`/`docs`/`local_cache` planned).
