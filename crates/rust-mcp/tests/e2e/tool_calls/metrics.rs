...
async fn rust_mcp_container_exports_tool_metrics_after_calls() {
    ...
    let _ = call_tool_response(&rust_mcp, "index_status", json!({})).await;

    // The /metrics endpoint is served on the main HTTP port, not a separate
    // Prometheus exporter port.
    let metrics = reqwest::Client::new()
        .get(format!("{}/metrics", rust_mcp.base_url()))
        .send()
        .await
    ...
    assert!(body.contains("tool=\"index_status\""));
