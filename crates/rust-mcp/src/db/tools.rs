...
pub async fn list_crate_impl_rows_for_filters(
    ...
    let mut base = String::from(
        "SELECT
            ...
            ci.trait_name_display,
            ci.impl_kind,
            COALESCE(NULLIF(ci.methods, 'null'::jsonb), '[]'::jsonb) AS methods,
            ci.is_blanket,
            ci.is_synthetic,
            ci.is_negative,
            ci.blanket_type,
...
pub async fn fetch_crate_type_info_row(
    ...
    sqlx::query_as::<_, CrateTypeInfoRow>(
        "SELECT
            ...
            ct.canonical_path,
            ct.definition_path,
            COALESCE(NULLIF(ct.generic_params, 'null'::jsonb), '[]'::jsonb) AS generic_params,
            COALESCE(NULLIF(ct.where_clauses, 'null'::jsonb), '[]'::jsonb) AS where_clauses,
            COALESCE(NULLIF(ct.fields, 'null'::jsonb), '[]'::jsonb) AS fields,
            COALESCE(NULLIF(ct.variants, 'null'::jsonb), '[]'::jsonb) AS variants,
            ct.deprecated_since,
            ct.deprecated_note,
